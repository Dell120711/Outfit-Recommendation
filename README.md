# Outfit-Recommendation

# Outfit Recommendation App (智慧穿搭推薦應用程式)

## 專案簡介 (Introduction)

本智慧穿搭推薦應用程式透過結合使用者上傳的衣物照片與個人檔案數據，提供客製化的穿搭建議，進而全面升級購物體驗。本系統利用先進的演算法，精準分析使用者的穿搭風格、身型尺寸及個人偏好，並針對各種不同場合提供最合適的服飾搭配推薦。

## 功能特點 (Features)

* **照片上傳**：使用者可以自由上傳個人衣物（如褲子、裙子等）的照片。
* **穿搭推薦**：系統會根據上傳的衣物照片，智慧推薦最合適的百搭上衣或下著。
* **個人檔案管理**：提供使用者註冊、登入及編輯個人專屬檔案的功能。
* **搭配歷史紀錄**：使用者可以隨時查看過去的穿搭組合，並收藏自己喜愛的搭配。
* **回饋機制**：使用者可對推薦結果提供意見回饋，協助系統持續優化與精進。

## Tech Stack

* **前端框架**：Next.js, Tailwind CSS, ShadCN
* **後端架報**：Next.js
* **資料庫服務**：Supabase
* **身分驗證**：OAuth
* **自動化測試**：Playwright
* **版本控制**：Git Flow
* **雲端部署**：Vercel

## 核心技術 (Core Technology)

我們的軟體核心在於利用LLM，對使用者上傳的照片進行深度分析並生成穿搭推薦

以下是系統運作的簡要概述：

Core Technology

我們的軟體利用先進的語言模型，根據使用者上傳的照片進行分析並提供穿搭推薦

以下是系統運作的簡要概述：

1. **影像分析 (Image Analysis)**：當使用者上傳衣物照片時，系統會利用計算機視覺技術處理影像，從中提取顏色、紋理和款式等特徵。
2. **特徵提取 (Feature Extraction)**：提取出的特徵會輸入至預先訓練、並針對時尚相關數據進行微調的語言模型中。該模型能夠理解該衣物的搭配語境與風格。
3. **推薦引擎 (Recommendation Engine)**：根據分析後的特徵，語言模型會推薦適合搭配的衣物。這些推薦結果是綜合考量時尚趨勢、使用者偏好以及風格相容性後生成的。
4. **回饋循環 (Feedback Loop)**：使用者可以對推薦結果提供意見回饋，這些數據將用於微調模型，進一步提升未來的推薦品質。

## Directory Structure

Getting Started
Follow these steps to set up the project locally:

1.Clone the repository:
```
git clone git@github.com:rmmfj/outfit-recommendation-app.git
cd outfit-recommendation-app
```
2.Install dependencies:
```
yarn
```
3.Set up environment variables: Create a `.env.local` file in the root directory and add your environment variables.

```env
DATABASE_URL=your_database_url
NEXT_PUBLIC_SUPABASE_ANON_KEY=your_database_anon_key
NEXT_PUBLIC_OPENAI_API_KEY=your_api_key
```

4.Run the development server:
```
yarn dev
```
5.Open your browser: Navigate to http://localhost:3000 to see the app in action.

```
/public
├── images
├── icons
/src
├── /app
│   ├── page.tsx (Home page)
│   ├── /upload
│   │   └── page.tsx (Upload photo page)
│   ├── /recommendation
│   │   └── page.tsx (Recommendation page)
│   ├── /register
│   │   └── page.tsx (User registration page)
│   ├── /login
│   │   └── page.tsx (User login page)
│   ├── /profile
│   │   └── page.tsx (User profile page)
│   ├── /history
│   │   └── page.tsx (Outfit history page)
│   └── /feedback
│       └── page.tsx (User feedback page)
├── /components
├── /styles
│   ├── globals.css
│   └── tailwind.css
├── /context
│   ├── auth-context.tsx
│   └── recommendation-context.tsx
├── /store
│   └── use-auth-store.ts (Zustand)
├── /hooks
│   ├── use-auth.ts
│   └── use-recommendations.ts
└── /utils
    ├── /supabase
    └── openai.ts
├── tailwind.config.js
├── next.config.js
├── tsconfig.json
├── package.json
└── README.md
```
